// ===============================
// ♀️ Venere Auroria — Motore Tecnico (RVA)
...
export function eseguiVenereAuroria(data, ora, luogo, comando) {
  ...
  if (!tipo) return { output: `Comando non riconosciuto. Usa: RVA` };

  // 1) Comandi Operativi (log tecnico)
  const operativi = applicaComandiOperativi('Venere Auroria');
  ...
  const gal = proiezioneGalatticaVenere(oraria, TABELLE_RVA);

  // 3.b (facoltativa) Legge Universale
  const legge = applicaLeggeUniversale?.({
    modulo: 'Venere Auroria',
  ...
  // 5) Output tecnico (stesso stile Auroria)
  const righe = [
    `✨ Metodo VENERE AURORIA attivo (${tipo})`,
    `📅 ${data} — 🕰️ ${ora} — 📍 ${luogo}`,
    ``,
    `🔭 ORARIA (reale)`,
...
// ===============================
// Proiezione galattica VENERE-centrica (stile Auroria)
// ===============================
const TABELLE_RVA = {
  GALASSIE: ['Andromeda', 'Sirio', 'Taurus', 'Michelaus (M41)'],
  // Compila con i tuoi NOMI UFFICIALI. Se mancano, avvisiamo ma non inventiamo.
  STELLE_PER_GALASSIA: {
...
function proiezioneGalatticaVenere(oraria, T) {
    ...
    if (!tabelleOK) {
      return {
        testo: [
          ...
          '• Tabelle galassie/stelle non impostate → compila TABELLE_RVA.',
        ].join('\n')
      };
    }
    return {
      testo: [
        '• Nessuna figura attiva Venere–pianeta.',
        '• (Definisci eventuale “galassia di quiete” per RVA nelle tue tabelle.)'
      ].join('\n')
    };
  }
  ...
  // mappa semplice giorno/notte come base (sostituibile con la tua regola specifica)
  ...
  return {
    ...
    testo: [
      ...
      '• Regola RVA: le stelle appartengono alla galassia; i pianeti forniscono la figura.',
    ].join('\n')
  };
}
