...
export function eseguiVenereEteria(data, ora, luogo, comando) {
  ...
  // 2️⃣–5️⃣ Metodi in sequenza tassativa
  ...
  // 6️⃣ Output tecnico (nessuna narrativa)
  const righe = [];
  ...
  console.log('✅ Venere Eteria eseguita correttamente:', outputFinale);
...
function metodoSaltoQuantico(oraria, gal, tz) {
  const out = [];
  const sun = oraria?.sunLon;
  const lilith = oraria?.planets?.LunaNera?.lon;
  ...
  const piani = ['Passato', 'Presente', 'Futuro'].map(label => {
    ...
    return `• ${label} → ancora ${fmtDeg(base)} — ${faro} — Figura: triangolazione energetica sui fasci attivi.`;
  });
...
function normalizzaLuogo(l) {
  ...
  return { name: l.trim(), lat: 45.776, lon: 12.056 };
}
function guessTZ() {
...
function bestAsp(delta, defs) {
  let best = null;
  for (const d of defs) {
    const diff = Math.abs(delta - d.g);
    if (diff <= d.orb && (!best || diff < best.diff)) best = { tipo: d.tipo, diff };
  }
  return best;
}
...
function oraLocaleHH(tz) {
  try {
    return Number(new Intl.DateTimeFormat('it-IT', { hour: '2-digit', hour12: false, timeZone: tz }).format(new Date()));
  } catch {
